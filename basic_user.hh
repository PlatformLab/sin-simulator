...
class BasicUser : public AbstractUser
{
    ...
    void get_best_slots(std::deque<struct Slot> &order_book, size_t flow_size)
    {
        ...
        recursive_slot_checker(order_book, 0, order_book.size(), flow_size, idxs_to_buy);
    ...
    private:
    int recursive_slot_checker(std::deque<struct Slot> &order_book, size_t start, size_t finish, size_t n,
    std::vector<size_t> &idxs)
    {
        ...
        for (size_t i = start; i < finish-n; i++)
        {
            ...
            int utility = -order_book.at(i).lowest_offer().cost
                + recursive_slot_checker(order_book, i+1, finish, n-1, recursive_idxs);
