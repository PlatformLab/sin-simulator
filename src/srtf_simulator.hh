...
const std::vector<Opportunity> simulate_srtf( const std::vector<Link> &links, std::vector<Flow> flows )
{
    assert ( links.size() == 1 );
    Link link = links.at(0);
    std::vector<Opportunity> allocation = link.get_opportunities();

    for ( Opportunity &opportunity : allocation ) {
        ...
        if ( srtf_flow != flows.end() ) {
            opportunity.owner = srtf_flow->uid;
